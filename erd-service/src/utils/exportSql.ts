...
function buildSubtypeDDL(entity: Entity, t: TFunc): { columnLines: string[]; checkLines: string[]; fkColumns: { column: Column; name: string }[] } {
  ...
  // 구분자(판별자) 컬럼
  columnLines.push(
    `  ${[q(discName), 'VARCHAR(30)', complete ? 'NOT NULL' : 'NULL', `COMMENT ${comment(t('sql.discriminatorComment', { name: discName }))}`].join(' ')}`
  );
...
export function generateMySQLDDL(entities: Entity[], t: TFunc): string {
  const byId = new Map(entities.map(e => [e.id, e]));
  ...
  const tables = sortParentsFirst(entities).map(entity => {
    ...
    const sub = buildSubtypeDDL(entity, t);
...
// .sql 파일 다운로드
export function exportDiagramSql(entities: Entity[], t: TFunc): void {
  if (entities.length === 0) return;
  const sql = generateMySQLDDL(entities, t);
